...
def create_true_data(config_params, freqs, N):
    ...
    dist = config_params['default']['dist']

    # beta and inc in the deafult parameters are provided as sin(beta) and cos(inc)
    if config_params['frame'] == 'LISA':
        ...
        t_ref, lam, beta, psi = LISA_to_SSB(config_params['default']['t_ref'], config_params['default']['lam'], np.arcsin(config_params['default']['beta']), config_params['default']['psi'], t0=0.)
    ...
    #else:
    #    print('No such reference frame')
    mbhb = wave_gen(xp.full((N), xp.array(m1)),xp.full((N), xp.array(m2)), xp.full((N), xp.array(a1)), xp.full((N), xp.array(a2)), xp.full((N), xp.array(dist))*PC_SI * 1e6, 
