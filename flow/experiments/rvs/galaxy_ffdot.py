...
class GalaxyFFdot(RV_base):
    ...
    def _log_prob(self, inputs_cupy):
        ...
        inputs = torch.zeros(inputs_nonorm.shape, device = self.dev)
        self.flow.eval()
        with torch.no_grad():
            inputs[:,0] = torch.log(inputs_nonorm[:,0])
            inputs[:,1] = torch.sign(inputs_nonorm[:,1])*torch.log10(torch.abs(inputs_nonorm[:,1]))
            ...
            log_prob_norm1_forward = cp.log(cp.log(10)) - cp.log(inputs_nonorm[:,0]) - cp.log(cp.abs(inputs_nonorm[:,1])) 
    ...
    def _renormalise(self, inputs):
        ...
        inputs[:,1] = -torch.sign(inputs[:,1])*torch.pow(10,-torch.abs(inputs[:,1]))
        return inputs
