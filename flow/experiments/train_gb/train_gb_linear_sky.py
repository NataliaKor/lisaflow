...
"""
from flow.experiments.data_generation.gbs.gb_model_onlysky import GB_gpu
...
def main(parser):
    ...
    ##############################
    # Switch off the embedding network completely to check the performance for a simple case 
   ...
   # plt.savefig('whiA.png')
    ...
    for j0 in range(number_epochs):
        ...
        with torch.no_grad():
            ...
            freq_sample_min = 0 # config_data['limits']['min']['fsamp']
            freq_sample_max = 1 #config_data['limits']['max']['fsamp']   
