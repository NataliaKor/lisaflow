...
"""
from data_generation.gbs.gb_model_freq import GB_gpu
...
# Scale frequencies 
def scale_freq(freq, freq_min, freq_max):
...
def main(parser):
    ...
    parser.add_argument('--config', type=str, default='configs/gbs/gb_resample_freq.yaml',
    ...
    for j0 in range(number_epochs):
        ...
        for i in range(number_iterations):
            ...
            param = gb.get_params()
            freqs = scale_freq(gb.get_freqs(), config_data['limits']['min']['fvec'], config_data['limits']['max']['fvec'])
       
            # Scale frequencies by the maximum frequency of the range on whih we analyse the data
            ...
            waveform = torch.cat((torch.as_tensor(cp.real(A)/maxA).type(dtype), torch.as_tensor(cp.real(E)/maxE).type(dtype), torch.as_tensor(cp.imag(A)/maxA).type(dtype), torch.as_tensor(cp.imag(E)/maxE).type(dtype), torch.as_tensor(freqs).type(dtype)), 1)
            #waveform = torch.cat((torch.as_tensor(cp.real(A)).type(dtype), torch.as_tensor(cp.real(E)).type(dtype), torch.as_tensor(cp.imag(A)).type(dtype), torch.as_tensor(cp.imag(E)).type(dtype), freqs), 1)
    
            waveform_cnn = torch.reshape(waveform, (waveform.shape[0], 5, 1, -1))
        ...
        with torch.no_grad():
            ...
            make_cp(flow, j, parameter_labels, np.array(param_min), param_max, waveform_cnn, truths, label)
