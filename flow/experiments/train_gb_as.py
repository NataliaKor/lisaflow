...
"""
from data_generation.gbs.gb_model_std_norm import GB_gpu
...
from flow.networks.resnet import ConvResidualNet, ConvResNet1D, Bottleneck, BasicBlock
...
def main(parser):
    ...
    parser.add_argument('--config_data', type=str, default='configs/gbs/gb_as.yaml',
    ...
    number_iterations = 2 #config['training']['max_iter']
    ...
    print('param_mean_cp = ', param_mean_cp)
    ...
    print('freqs_arr = ', freqs_arr)
    print('freqs_arr[0,:] = ', freqs_arr[0,:])
    print('freqs_arr[1,:] = ', freqs_arr[1,:])
    # TEST WAVEFORMS
    test_wf = False
    if test_wf == True:
        index = 0
        ...
        waveform = torch.cat((torch.as_tensor(cp.real(A)/max_batch).type(dtype),
                              ...
                              torch.as_tensor(cp.imag(A)/max_batch).type(dtype), freqs_one), 1)
        print('waveform.shape = ', waveform.shape)
        waveform_cnn = torch.reshape(waveform, (waveform.shape[0], embed_in, -1)) 
    ...
    for j0 in range(number_epochs):
        ...
        for i in range(number_iterations):
            ...
            waveform = torch.cat((torch.as_tensor(cp.real(A)/max_batch).type(dtype).view(batch_size, 1, -1), 
                                  torch.as_tensor(cp.real(E)/max_batch).type(dtype).view(batch_size, 1, -1), 
                                  torch.as_tensor(cp.imag(A)/max_batch).type(dtype).view(batch_size, 1, -1), 
        ...
        with torch.no_grad():
            ...
            waveform = torch.cat((torch.as_tensor(cp.real(A)/max_batch).type(dtype).view(1, 1, -1), 
                                  torch.as_tensor(cp.real(E)/max_batch).type(dtype).view(1, 1, -1), 
                                  torch.as_tensor(cp.imag(A)/max_batch).type(dtype).view(1, 1, -1), 
            ...
            for idx in tqdm(range(neval)):
                ...
                #freqs_samp = torch.as_tensor(cp.tile(freqs,(num_samples, 1))).type(dtype).view(1, 1, -1) 
                waveform = torch.cat((torch.as_tensor(cp.real(A)/max_batch).type(dtype).view(1, 1, -1), 
                                      torch.as_tensor(cp.real(E)/max_batch).type(dtype).view(1, 1, -1), 
                                      torch.as_tensor(cp.imag(A)/max_batch).type(dtype).view(1, 1, -1), 
                                      torch.as_tensor(cp.imag(E)/max_batch).type(dtype).view(1, 1, -1), freqs_one), 1)
