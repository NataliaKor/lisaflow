...
def main(parser):
    ...
    for j0 in range(number_epochs):
        ...
        for i in range(number_iterations):
            ...
            coeff_scale = (coeff + noise_samples)/torch.sqrt(Sts_reduce + 1.0)
        ...
        with torch.no_grad():
            ...
            coeff_true = torch.matmul(wf_true, Vts_reduce)
            noise_samples = torch.randn(coeff_true.shape).type(dtype)
            coeff_scale_true = (coeff_true + noise_samples)/torch.sqrt(Sts_reduce + 1.0)
