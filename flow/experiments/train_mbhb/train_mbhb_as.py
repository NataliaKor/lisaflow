...
from data_generation.mbhbs.mbhb_model_vary_time import MBHB_gpu
...
def main(parser):
    ...
    if config['svd']['estimate'] and (config['training']['resume'] == 0):
        ...
        print(config['svd']['root'] + config['svd']['path'])
    ...
    #    print('Vts_reduce.shape = ', Vts_reduce.shape)
    #    print('Sts_reduce.shape = ', Sts_reduce.shape)
    #    print('type(Vts_reduce) = ', Vts_reduce)
    #else:
    #    fin = h5py.File(config['svd']['root'] + config['svd']['path'], 'r')
    #    Sts = fin['S']
    #    Vts = fin['V']
    ...
    mbhb.sample_from_prior(Nsamples, 0)
    #mbhb_ts = mbhb.create_waveform()
    #param_mean = mbhb.get_param_mean()
    #param_std = mbhb.get_param_std()
