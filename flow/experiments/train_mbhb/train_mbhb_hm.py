...
from flow.experiments.data_generation.mbhbs.mbhb_true_data_hm import create_true_data
from flow.experiments.data_generation.mbhbs.mbhb_model_hm import MBHB_gpu

from cuml.decomposition import IncrementalPCA
from cuml.decomposition import PCA
from cuml.decomposition import TruncatedSVD
#from cuml.decomposition import UMAP
...
def main(parser):
    ...
    parser.add_argument('--config', type=str, default='../configs/mbhbs/pe/mbhb_network_spritz2.yaml',
                        help='Path to config file specifying model architecture and training procedure')
    parser.add_argument('--config_data', type=str, default='../configs/mbhbs/pe/mbhb_params_spritz2_fix.yaml',
    ...
    # Load config
    config = get_config_yaml(args.config)
    config_params = get_config_yaml(args.config_data)
    ...
    for key, value in config_params['estimate'].items():
        features_size = features_size + value
    ...
    mbhb = MBHB_gpu(config, config_params, dtype)
    ...
    if config['svd']['estimate'] and (config['training']['resume'] == 0):
        if config['svd']['method'] == 'ipca':
            ...
            ipca = IncrementalPCA(n_components=config['svd']['n_components'], batch_size = batch_pca)
        ...
        elif config['svd']['method'] == 'pca':
  
             mbhb.sample_from_prior(500, 1)
             mbhb.freqwave_AE()
             mbhb_ts = mbhb.timewave_AE()
             pca = TruncatedSVD(n_components=config['svd']['n_components'],algorithm = "jacobi", n_iter = 20, tol = 1e-9)
             pca.fit(mbhb_ts)
             print(cp.cumsum(pca.explained_variance_ratio_))
             print('S.shape = ', pca.singular_values_.shape)
             print('V.shape = ', pca.components_.shape)
             Vts = cp.transpose(pca.components_).get()
             Sts = pca.singular_values_.get()

        else:

            # Initialise a class for MBHB
            N_pca = 10000
            mbhb.sample_from_prior(N_pca, 1)
            mbhb.freqwave_AE()
            mbhb_ts = mbhb.timewave_AE()
        ...
        f = h5py.File(config['svd']['root'] + config['svd']['path'] + config['saving']['label'] + '.hdf5', 'w')
        ...
        print(config['svd']['root'] + config['svd']['path'] + config['saving']['label'] + '.hdf5')
        fin = h5py.File(config['svd']['root'] + config['svd']['path'] + config['saving']['label'] + '.hdf5', 'r')
        Sts = fin['S']
        Vts = fin['V']
    ...
    for j0 in range(number_epochs):
        ...
        for i in range(number_iterations):
            ...
            mbhb.freqwave_AE()
            mbhb_ts = torch.as_tensor(mbhb.timewave_AE()).type(dtype)
            
            noise_samples = torch.randn(mbhb_ts.shape).type(dtype)
            coeff_scale = torch.matmul(mbhb_ts + noise_samples, (1.0/torch.sqrt(Sts_reduce + 1.0))*Vts_reduce)
...
#                    optim_type = 1 
        ...
        plt.plot(losses)
        plt.plot(losses_val)
        plt.savefig('../plots/losses_' + config['saving']['label'] + '.png')
        plt.close()
        ...
        with torch.no_grad():
            ...
            wf_true_cp, truths = create_true_data(config_params, mbhb.get_freqs())
            wf_true = torch.as_tensor(wf_true_cp).type(dtype)
            #noise_samples = torch.randn(wf_true.shape).type(dtype)
 
            coeff_scale_true = torch.matmul(wf_true, (1.0/torch.sqrt(Sts_reduce + 1.0))*Vts_reduce)
            ...
            # Values to renormalise parameters back
            param_mean = torch.as_tensor(mbhb.get_param_mean()).type(dtype)
            param_std = torch.as_tensor(mbhb.get_param_std()).type(dtype)
            #param_min = torch.as_tensor(mbhb.get_param_min()).type(dtype)
            #param_max = torch.as_tensor(mbhb.get_param_max()).type(dtype)
            ...
            make_cp_hm(flow, j, parameter_labels, param_mean, param_std, coeff_scale_true, truths, label)
