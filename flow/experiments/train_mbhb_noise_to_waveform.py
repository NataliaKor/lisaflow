...
#def true_data(config_data, Sts_reduce, Vts_reduce, dtype):
#    '''
...
def main(parser):
    ...
    parser.add_argument('--config', type=str, default='configs/mbhbs/mbhb_resample_noise_to_waveform.yaml',
                        help='Path to config file specifying model architecture and training procedure')
    parser.add_argument('--config_data', type=str, default='configs/mbhbs/mbhb_data_radler.yaml',
    ...
    for j0 in range(number_epochs):
        ...
        for i in range(number_iterations):
            ...
            # Add noise to coeff
            # TODO 1) check if +1 is in the correct place
            # TODO 2) compare adding noise to the waveforms and to the coefficients

            coeff = torch.matmul((mbhb_ts + noise_samples), Vts_reduce) 
 
            coeff_scale = coeff/torch.sqrt(Sts_reduce + 1.0)
        ...
        with torch.no_grad():
            ...
            coeff_scale_true = coeff_true/torch.sqrt(Sts_reduce + 1.0)
