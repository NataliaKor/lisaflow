...
from flow.flowtransforms import splines
from flow.flowtransforms.base import Transform
...
class AffineCouplingTransform(CouplingTransform):
    ...
    GENERAL_SCALE_ACTIVATION = lambda x : (softplus(x) + 1e-3).clamp(0, 1.2)

    # TODO make it a parameter that is specified externally
    def __init__(self, mask, transform_net_create_fn, unconditional_transform=None, scale_activation=GENERAL_SCALE_ACTIVATION):
