...
class AnalyticNoise(ABC):
    ...
    def __init__(self, frq, noise_type):
        ...
        self.freq = cp.array(frq)           

        # Acceleration noise
        Sa_a = self.DSa_a * (1.0 +(0.4e-3/self.freq)**2) * (1.0+(self.freq/8e-3)**4) # in acceleration
        self.Sa_d = Sa_a*(2.*cp.pi*self.freq)**(-4.) # in displacement
        Sa_nu = self.Sa_d*(2.0*cp.pi*self.freq/CLIGHT)**2 # in rel freq unit
        self.Spm =  Sa_nu

        # Optical Metrology System
        self.Soms_d = self.DSoms_d * (1. + (2.e-3/self.freq)**4) # in displacement
        Soms_nu = self.Soms_d*(2.0*cp.pi*self.freq/CLIGHT)**2 # in rel freq unit
        self.Sop =  Soms_nu
    ...
    def psd(self, option="A", tdi2=False):#, includewd=0):
        ...
        x = 2.0 * cp.pi * lisaLT * self.freq # make this CPU/GPU agnostic 
